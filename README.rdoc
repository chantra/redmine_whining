...
This rake job can be used to make sure that a ticket does not get forgottent about by emailing the owner of a ticket
 when tickets are active and have not been updated since a certain amount of days.

==Compatibility

This plugin is compatible with Redmine 0.9.x, Redmine 1.X as well as with the current Redmine trunk.

==Obtaining 

<i>URL of the GitHub repository: http://github.com/chantra/redmine_whining/tree/master</i>
...
<tt>$ git branch 0.0.X</tt>

<b>Only branch 0.0.X is supported for now. Master branch is rather unstable</b>

==Installation

- Download the plugin from the available source (see http://redmine.debuntu.org/projects/redmine-whining/wiki )
...
- Backup the currently deployed Whining plugin (<tt>cp -r /vendor/plugins/redmine_whining /vendor/plugins/redmine_whining-backup</tt>)
...
If you would like to report a bug or request a new feature you can open a new issue at the issue-tracking page of the plugins' http://redmine.debuntu.org/projects/redmine-whining/issues .
