...
class WhiningMailer < Mailer
  ...
  def self.whinings(options={})
    ...
    s = ARCondition.new ["#{IssueStatus.table_name}.is_closed = ? AND #{Issue.table_name}.updated_on <= ? AND #{Issue.table_name}.assigned_to_id IS NOT NULL AND #{Project.table_name}.status=?", false, days.day.until.to_date,1]
